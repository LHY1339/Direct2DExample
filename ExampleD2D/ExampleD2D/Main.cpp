...
void DrawD2DImage()
{
	...
	const float width = (round_rect_size.rect.right - round_rect_size.rect.left) / bitmapSize.width;
	const float height = (round_rect_size.rect.bottom - round_rect_size.rect.top) / bitmapSize.height;
	D2D1_MATRIX_3X2_F trans = D2D1::Matrix3x2F::Translation(round_rect_size.rect.left, round_rect_size.rect.top);
	D2D1_MATRIX_3X2_F scale = D2D1::Matrix3x2F::Scale(width, height, D2D1::Point2F(round_rect_size.rect.left, round_rect_size.rect.top));
